...
只有当候选提交已经通过 CI、关键 Windows 双击流程完成一次人工验收，并且没有未解释的失败或安全告警时，才将 `0.2.6.dev0` 改为正式版本并创建标签。不要为了赶宣传时间跳过这一步。
...
1. 将 `pyproject.toml` 的版本从开发版本改为正式版本，例如 `0.2.6`，并将变更记录移入带日期的版本标题。
2. 再次执行完整发布验收，提交并推送 `main`，等待该提交的 CI 通过。
3. 创建完全匹配的标签，例如 `v0.2.6` 并推送。Release 工作流会校验标签与包版本一致、执行发布门槛、构建 wheel/sdist、生成 SHA256SUMS，并创建 GitHub Release。
