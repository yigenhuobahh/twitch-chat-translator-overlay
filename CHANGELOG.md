...
- Full-codebase review wave (2026-09-12, 6-lens audit + fix waves; 33 findings fixed, 2 structural refactors deliberately deferred with repro evidence, 1 claim refuted by its own mutation test):
  - **Orphan processes / lifecycle**: the TUI's pipeline child now registers in the shared process table and the TUI installs the cleanup handlers, so `taskkill /F`/SIGTERM/crash no longer orphans an hours-long ffmpeg tree (job_wizard's documented contract finally holds on the TUI path too); the decode-check registers its child before its first disk write, shrinking the kill-while-spawn window; cancel-then-immediate-restart no longer leaves a zombie `running` history record; a history lock held >5s by a wedged second TUI instance now degrades with a status hint instead of crashing on `mark_running`/clear-history.
  - **Data loss**: the download transaction's publish/restore renames go through the shared Windows-sharing-violation retry helper, so a player holding the old `video.mp4` no longer aborts a finished multi-GB download (the recovery path had been deleting the staged video + chat together); a failed tool-install rollback keeps the original error in the chain and prints where the old install was preserved; the sensitive translation-context handoff file is now claimed by `--clean` residue rules after a hard kill.
  - **Redaction**: an `-`-prefixed OAuth token rejected by the download guard is no longer echoed into the exception message (a Chinese-key rule also joined `redact_text`); the wizard's generic download-exception branch, the standalone translator's batch-retry log and the `--doctor` Base-URL line now pass through `redact_text` like their sibling paths already did.
  - **Silent-wrong output**: job YAML fields containing U+0085/U+2028/U+2029 can no longer produce a permanently unparseable file (quoted + escaped like \n); preset numeric fields validate positivity/finite-ness on both string and number paths (bool rejected in float layout fields); single-line truncation budgets subtract the real header width so the ellipsis never overruns the bitmap edge.
  - **Memory / performance**: message-bitmap pre-rendering gained a byte budget (512 MB eager cap with on-demand fallback, 256 MB per-bitmap re-truncation) mirroring the emote decode budget — the 200-line cap alone could still stack ~11 GB at 999 messages; progress/final translation JSON and `atomic_write_json` switched to compact serialization (a 1 M-message run was measured spending ~3 h of pure serialization under the progress lock); review XLSX export/import moved to openpyxl write-only/read-only streaming (1 M rows: ~5 GB RSS → roughly constant, per-row heights skipped past 50k); `normalize_translation` no longer rewrites the JSON when zero rules fired; the retry-missing-translation tail runs concurrently like the main phase (measured 3.85× wall-clock inversion at 4 workers); cache keys pre-digest the glossary-sized context once per run instead of re-hashing it per message (v2 keys namespace-isolated from v1); the parser no longer pre-scans the whole HTML body just to print an approximate block count.
  - **CLI / config surface**: rational fps strings (`"30000/1001"`) arriving via `--render-preset` at the burn entry point normalize through the shared parser instead of dying with a float-conversion error; `--allow-empty-chat` is now a registered pipeline flag (job YAML/TUI/CLI forward it like `--keep-temp`) after the burn error message had been pointing users at a flag only the low-level engine accepted; `--clean` claims only tool-artifact `.bak` forms (`*.mp4.bak` etc.) instead of every user-made `*.bak`, and the `--clean` help texts plus both READMEs state the real scope.
  - **Consistency / dead code**: the download single/multi-segment health-check/repair/publish blocks and staging-cleanup tails collapsed into shared helpers (call-site behavior preserved verbatim, differences passed as explicit parameters); two never-triggered `try: from process_util import … except ImportError` guards in `twitch_download` are gone; the three hand-copied path-flag lists in `job_wizard` derive from one source with a same-origin guard test.
  - **Tests / docs**: the `sys.modules.pop("tui_run")` re-import test now restores in `finally` (the 0906 lesson applied to its second half); Windows `taskkill /T` command shape gets a forced-branch test; 13 doctor/readiness tests snapshot `PATH` before triggering `prepend_tools_*` (0905 lesson); `vod_merge` merge guards (empty segments/timeline mismatch/2 GiB refusal) and `exclusive_file_lock` symlink/irregular-file guards gained direct tests; `detect_cjk_font` covers all three platform branches via forced `platform.system`, so a typo'd candidate table can no longer pass silently; emote class matching gained a substring-counterexample test; `CLAUDE.md`/`CONTRIBUTING.md` claims corrected against reality (40+ new regression tests; suite 1560+ green, ruff/bandit clean).
...
- Full-codebase review wave (2026-09-05, 3 critical + 9 required + optional findings):
  - **Security**: `run_cli.bat`/`doctor.bat` reject cmd metacharacters in arguments, so a file named `a&calc&b.mp4` dragged onto the launcher can no longer execute arbitrary commands; a working-directory `.env` supplying both a translation endpoint and an API key now requires interactive confirmation (non-TTY runs reject it), preventing chat text from being silently redirected to a third-party endpoint; review XLSX/TSV export prefixes formula-looking cells (`= + - @` tab) with `'` and strips it on import; chat HTML above 2 GiB is refused at all three read entry points; rules YAML `preserve_patterns` longer than 500 chars are rejected at compile time; run_meta `argv` dumps pass through command redaction so `--oauth` values never reach disk; `.env` gets `0600` on POSIX.
  - **Data loss**: translation JSON edited with a BOM (PowerShell 5.1/Notepad) no longer defeats the nonempty-count guard (utf-8-sig everywhere), so reviewed translations cannot be silently wiped by a re-export; lint severity merging is monotonic — an `OK` issue can no longer downgrade an accumulated `WARN`.
  - **Silent-wrong output**: TUI one-click production no longer truncates the render to the 10-second preview (`--preview-clip` is projected only for preview modes and the form hides the field for production modes); valid-but-empty model responses now take the in-batch retry path; AUTH/CLIENT terminal errors abort remaining batches instead of hammering the endpoint.
  - **Robustness**: job YAML numeric fields are validated against range tables aligned with the burn CLI (`msg_lifetime: 0` fails at load, not after a wasted translation round); the SIGINT handler takes its lock non-blocking (no more Ctrl+C deadlock); run_meta liveness has a 7-day absolute cap so a reused Windows pid cannot pin a crashed job as live forever; `--clean` recognizes `.download-*` orphan staging files; doctor reads the API triple through the shared config source (legacy `AGNES_*` fallback restored); fixed-tmp writers moved to unique mkstemp names; doctor/env-consistency, glob escaping for preview artifact lookup, download probe-failure warnings, and the full list live in the individual commits (`f0a4665`..`245304b`).
